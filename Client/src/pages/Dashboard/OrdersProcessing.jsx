...
function ProcessingOrders() {
 ...
 useEffect(() => {
  ...
  const getId = (order, index = 0) => {
    return order.id ?? order.order_id ?? index;
  };

  const getCustomer = (order) => {
    return order.consignee_name || order.customer_name || order.name || "—";
  };
  ...
  const getDeliveryCity = (order) => {
    return order.city || order.delivery_city || order.deliveryCity || "—";
  };
  ...
  const getWeightNumber = (order) => {
    if (order.total_weight != null) {
      return Number(order.total_weight) || 0;
    }

    if (order.weight != null) {
      return Number(order.weight) || 0;
    }

    if (order.dead_weight != null) {
      return Number(order.dead_weight) || 0;
    }
  ...
  const getVolumetricWeight = (order) => {
    ...
    if (length > 0 && width > 0 && height > 0) {
      ...
      return `VWt: ${volumetricWeight.toFixed(2)} kg`;
    }
  ...
  const getBoxCount = (order) => {
    if (Array.isArray(order.packages) && order.packages.length) {
      const total = order.packages.reduce((sum, pkg) => {
        return sum + Number(pkg?.package_count ?? pkg?.count ?? 1);
      }, 0);
  ...
  const toggleAll = () => {
    ...
    setSelectedOrders((previous) => [...new Set([...previous, ...visibleIds])]);
  };
  ...
  const calculatePreview = async (orderList, serviceType = shippingType) => {
    ...
    try {
      ...
      console.log("Preview rate error:", error);
  ...
  const confirmShipNow = async () => {
    ...
    if (previewLoading) {
      showToast("Please wait while shipping charges are calculated.", "error");
...
window.dispatchEvent(new Event("orderStatusUpdated"));
      ...
      console.log("Bulk ship error:", error);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!result?.success) {
        throw new Error(result?.message || "Unable to delete orders");
      }
          ...
          <table className="w-full min-w-[1160px] table-fixed border-collapse">
            ...
            <tbody>
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan="7" className="h-[280px] text-center">
                    <div className="flex flex-col items-center justify-center">
                      <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-slate-100">
                        <Icon name={searchText ? "search" : "file"} size={21} />
                      </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="border-r border-b border-slate-200 px-4">
                        ...
                        <div className="mt-[5px] text-[10px] leading-[15px] text-slate-400">
                          <div>{getDate(order)}</div>

                          <div>{getTime(order)}</div>
                        </div>
                      </td>
                      ...
                      <td className="border-r border-b border-slate-200 px-4">
                        <div className="text-[12px] text-slate-700">
                          {getBoxCount(order)} Box × {getTotalWeight(order)}
                        </div>

                        <div className="text-[11px] text-slate-500">
                          Wt: {getTotalWeight(order)}
                        </div>
                      </td>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
              ...
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="rounded-full bg-[#f2edff] px-3 py-1 text-[11px] font-medium text-[#6847ed]">
                  {selectedOrderObjects.length} Selected
                </span>
              </div>
                ...
                <div className="flex min-h-[180px] items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-xs font-medium text-slate-500">
                      Calculating{" "}
                      {shippingType === "AIR" ? "Air" : "Delivery By Road"}{" "}
                      shipping charges...
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-1 text-[10px] text-slate-500">
                      ...
                      {selectedOrderObjects.length !== 1 ? "s" : ""}
                    </div>
